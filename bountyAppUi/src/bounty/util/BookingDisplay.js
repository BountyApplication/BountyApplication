...
export function ProductList({className, products, allProducts, setProducts}) {
    // var count = 1;
    return(
            ...
            <tbody>
                {products!=null && products.map(({productId, name, price, amount}) => 
                    <tr key={productId}>
                        {/* <td>{count++}</td> */}
                        <td>{name}</td>
                        <td>{amount}<Button variant="primary" className='ms-3 px-2 pb-1 pt-0' onClick={()=>setProducts(allProducts.map((product) => product.productId!==productId?{...product}:{...product, amount: amount-1}))}>-</Button></td>
                        <td>{`${price.toFixed(2)}€`}</td>
                        <td>{`${(price*amount).toFixed(2)}€`}<Button variant="danger" className='ms-3 px-2 pb-1 pt-0' onClick={()=>setProducts(allProducts.map((product) => product.productId!==productId?{...product}:{...product, amount: 0}))}>X</Button></td>
                    </tr>
                )}
            </tbody>
        </Table>
...
export default function BookingDisplay({children, booking: {oldBalance, newBalance, productSum, correction, cashPayment, products}, allProducts, setProducts, isHistory = false}) {
    ...
    return <Container className="d-flex h-100 align-items-center flex-column">
        ...
        <Collapse in={products!=null&&products.length!==0}><div className='overflow-auto w-100'><ProductList className="mt-3" products={products.filter(({amount}) => amount!==0)} allProducts={allProducts} setProducts={setProducts} /></div></Collapse>
